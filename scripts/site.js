...
Y.use('node','squarespace-gallery-ng', function(Y) {
  ...
  Y.on('domready', function() {
    ...
    // if(window.location.origin == 'https://secure.squarespace.com') {
    ...
    _setupBottomBar: function() {
      if (!Modernizr.touch && Y.Squarespace.Template.getTweakValue('autohide-footer') + "" === "true") {
        Y.one(window).on('mousemove', function(e) {
          if(Y.one('#bottomBar')){
            if (e.clientY > Y.one('body').get('winHeight') - Y.one('#bottomBar').height()) {
              Y.one('#bottomBar').addClass('viewable');
            } else {
              Y.one('#bottomBar').removeClass('viewable');
            }
          }
